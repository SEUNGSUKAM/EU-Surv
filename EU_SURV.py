...
class Model_Longitudinal_Attention:
    ...
    def __init__(self, sess, name, input_dims, network_settings):
        print("Initiating the Longitudinal_Attention model with Event Prediction")
        self.sess               = sess
        self.name               = name
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            self.lambda_tail  = tf.placeholder(tf.float32, name='lambda_tail')
            ...
            self.fc_mask_tail = tf.placeholder(tf.float32, shape=[None, self.num_Event, self.num_Category], name='fc_mask_tail')
            ...
            self.out_event_logit  = FC_Net(h, self.num_Event + 1, activation_fn=None, weights_initializer=self.initial_W, weights_regularizer=self.reg_W_out, scope="Output_Event")
            self.out_event = tf.nn.softmax(self.out_event_logit, axis=1)
            ...
            self.loss_event()            #get loss4: Event prediction loss
            
            
            self.LOSS_TOTAL     = self.a*self.LOSS_1 + self.b*self.LOSS_2 + self.c*self.LOSS_3 + tf.losses.get_regularization_loss() + self.LOSS_EVENT
    ...
    def loss_event(self):
        self.LOSS_EVENT = tf.reduce_mean(
                                            tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.cast(tf.squeeze(self.k), tf.int32), logits=self.out_event_logit)
        )
    ...
    def loss_Log_Likelihood(self):
        ...
        tail_mass = tf.reduce_sum(tf.reduce_sum(self.fc_mask_tail * self.out, axis=2), axis=1, keepdims=True)
        tail_term = self.lambda_tail * I_1 * log(tail_mass)  # I_1: event indicator, log() already epsilon-safe

        self.LOSS_1 = - tf.reduce_mean(tmp1 + sigma3*tmp2 + tail_term)
    ...
    def loss_Ranking(self):
        ...
        for e in range(self.num_Event):
            ...
            #tmp_eta = tf.reduce_mean(tmp_eta * self.out_event[:, e, :], reduction_indices=1, keepdims=True)
            eta.append(tmp_eta)
    ...
    def get_cost(self, DATA, MASK, MISSING, PARAMETERS, keep_prob, lr_train):
        (x_mb, k_mb, t_mb)        = DATA
        (m1_mb, m2_mb, m3_mb, m_tail_mb)     = MASK
        (x_mi_mb)                 = MISSING
        (alpha, beta, gamma, tail_lambda)      = PARAMETERS
        return self.sess.run([self.LOSS_TOTAL,  self.LOSS_1, self.LOSS_2, self.LOSS_3, self.LOSS_EVENT], 
                             feed_dict={self.x:x_mb, self.x_mi: x_mi_mb, self.k:k_mb, self.t:t_mb, 
                                        self.fc_mask1: m1_mb, self.fc_mask2:m2_mb, self.fc_mask3: m3_mb, self.fc_mask_tail: m_tail_mb,
                                        self.a:alpha, self.b:beta, self.c:gamma, self.lambda_tail:  tail_lambda,
    ...
    def predict(self, x_test, x_mi_test, keep_prob=1.0):
        return self.sess.run([self.out, self.out_event], feed_dict={self.x: x_test, self.x_mi: x_mi_test, self.mb_size: np.shape(x_test)[0], self.keep_prob: keep_prob})
