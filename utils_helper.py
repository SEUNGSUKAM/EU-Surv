...
def f_get_minibatch(mb_size, x, x_mi, label, time, mask1, mask2, mask3, seed):
...
def f_get_minibatch_tail(mb_size, x, x_mi, label, time, mask1, mask2, mask3, mask_tail, seed):
    idx = range(np.shape(x)[0])
    random.seed(seed)
    idx = random.sample(idx, mb_size)
    ...
    m2_mb    = mask2[idx, :, :].astype(float) #fc_mask
    m3_mb    = mask3[idx, :].astype(float) #fc_mask
    m_tail_mb = mask_tail[idx, :].astype(float) #fc_mask
    return x_mb, x_mi_mb, k_mb, t_mb, m1_mb, m2_mb, m3_mb, m_tail_mb


def f_get_boosted_trainset(x, x_mi, time, label, mask1, mask2, mask3, mask_tail):
    ...
    new_mask_tail      = np.zeros([total_sample, np.shape(mask2)[1], np.shape(mask_tail)[2]])
    ...
    for i in range(np.shape(x)[0]):
        ...
        for t in range(max_meas):
            ...
            new_mask_tail[tmp_idx+t, :, :] = mask_tail[i, :, :]
            new_mask3[tmp_idx+t, :]    = mask3[i, :]

        tmp_idx += max_meas
        
    return(new_x, new_x_mi, new_time, new_label, new_mask1, new_mask2, new_mask3, new_mask_tail)
